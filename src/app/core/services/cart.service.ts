  ...
  constructor( private config:ConfigService) {
    ...
    if(cart){
      ...
      if(carritoGuardado){
        ...
        if (this.config.configuracion()) {
          const diasVencimiento = this.config.configuracion().diasVencimientoCarrito;
          if (fecha.getTime() - fechaGuardado.getTime() > 1000 * 60 * 60 * 24 * diasVencimiento) {
            this.vaciar();
          }     
        }else{
